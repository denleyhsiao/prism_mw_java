...
public class SocketDistribution extends AbstractDistribution
{
    ...
    */     
    ...
    public boolean disconnect(String hostName, int portNum) 
    {                
        for (int i=0; i<= conns.size(); i++)
        {
            Connection conn = (Connection)conns.elementAt(i);
            InetAddress thisHost = conn.getHost();
            String thisHostIP = thisHost.getHostAddress();
            String thisHostName = thisHost.getHostName();
            int port = conn.getPort();
            if ((thisHostIP.equals(hostName) || thisHostName.equals(hostName)) && port == portNum)
            {
                conn.teardown();
                conn.close();
